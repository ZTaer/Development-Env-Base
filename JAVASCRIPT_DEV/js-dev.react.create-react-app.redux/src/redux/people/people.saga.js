...
import { fetchPeopleFailure, fetchPeopleSuccess } from "./people.action";
...
export function* fetchPeopleReduxSagaStart() {
    try {
        ...
        yield delay(2000);
        console.log(
            "Redux Saga 通过 select 获取reselect数据: ",
            yield select(selectPeopleOrigin)
        );
    } catch (err) {
...
export function* onFetchPeopleReduxSagaStart() {
    yield takeLatest(
        peopleActionTypes.FETCH_PEOPLE_REDUX_SAGA_START,
        fetchPeopleReduxSagaStart
    );
}
export function* onFetchPeopleReduxSagaTimeOutStart() {
    yield takeLatest(
        peopleActionTypes.FETCH_PEOPLE_SUCCESS,
        fetchPeopleReduxSagaTimeOutStart
    );
}
